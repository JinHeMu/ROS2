...
{
public:
    RotateWheelNode(const std::string &name)
        : Node(name), joint_speeds_({0.0, 0.0})
    {
        ...
        timer_ = this->create_wall_timer(
            ...
            std::bind(&RotateWheelNode::timer_callback, this)
        );
    ...
    void update_speed(const std::vector<double> &speeds)
    {
        ...
        {
            RCLCPP_ERROR(this->get_logger(), "Invalid speeds size. Expected size: 2");
        }
    }
...
private:
    ...
    void timer_callback()
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Published joint states: position=[%f, %f], velocity=[%f, %f]",
                    joint_states_.position[0], joint_states_.position[1],
                    joint_states_.velocity[0], joint_states_.velocity[1]);
    }
